...
def main():
    ...
    base_model.load_state_dict(torch.load('Copy of resnet50_celeba.model'))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(text_encoder):
                ...
                latents = vae.encode(images.to(dtype=weight_dtype)).latent_dist.sample().detach()
                latents = latents * vae.config.scaling_factor
                scale = int(2 ** (len(vae.config.block_out_channels) - 1))
                #print(int(2 ** (len(vae.config.block_out_channels) - 1)), (2 ** (len(vae.config.block_out_channels) - 1)))
                ...
                img_latent = noisy_latents
                # Predict the noise residual
                model_pred = unet(img_latent, timesteps, encoder_hidden_states).sample
